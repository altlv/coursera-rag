...
### Personality, and the null result that was my fault

A fair observation from a user: the answers read like citations from the documentation rather than replies to a question — *"a glorified search box"*. True, and a direct consequence of `Answer ONLY using the numbered context passages`.

So three answer styles were added, with the **grounding rules identical across all of them**, byte for byte, enforced by a test. A style may change how an answer is organised and how it addresses the reader; it may not change what counts as a supported claim, when to refuse, or how to cite.

**They changed nothing.** Same opening sentence, same wording, across every style. I varied the prompts, the rule order, the temperature (0.2 vs 0.7) and the model (`gpt-4o-mini` vs `llama-3.3-70b`) — nothing moved. So I wrote up a confident explanation: personality and strict grounding are in tension, and grounding wins by construction.

**That explanation was wrong**, and the same user found it out in one question: *would a radical style — lolcat — change anything?*
...
Completely transformed, grounding untouched. ⚙️ **The mechanism was never blocked. My instructions were adjectives.**
...
A model already believes it writes clearly and answers directly, so asking for those is a no-op. Rewritten as behaviours, the `tutor` style now opens *"You may struggle to manage and share code effectively across your application as it grows"* — the problem, before the feature, which is exactly what the first version asked for politely and never got.

⚙️ The transferable lesson is not about prompts: **a null result means nothing until you have shown the mechanism can respond at all.** An absurd dose is the cheapest possible check, and I had written a false explanation into this file for want of running it. It is now the [positive-control section](TESTING-A-RAG.md#5-a-null-result-with-no-positive-control-is-not-a-result) of the testing guide.

**The silly voices shipped**, and not only as a joke. They make a property visible on every use: the facts, citations and refusals are identical whether the assistant sounds like a reference manual or a cat. If a voice ever *did* change what the assistant claims, that would be a grounding bug — so the control stays in the product.

One real defect surfaced while wiring this up, and it is the other recurring shape: every style test passed while `buildPrompt` ignored the style entirely and used a hard-coded prompt. The styles were correct, the assembly was correct, **and nothing connected them**. Testing a pure function proves the function, never that anything calls it.
