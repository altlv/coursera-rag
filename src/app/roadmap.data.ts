  ...
  // -------------------------------------------------------------------------
  {
    ...
    tasks: [
      ...
      },
      {
        ...
        detail:
          'Answers arrive as they are written, over SSE. The plumbing is unremarkable; the trade-off is not. EVERY output-side guard runs after the model finishes - the injection detector can refuse a whole answer and citation stripping edits the text - so streaming genuinely weakens the output half of the injection defence. The guard splits into two kinds. Known-payload patterns are INCREMENTAL - never legitimate at any length - so they run on the accumulated text before each piece is forwarded, and a payload is never displayed, including one complete in the first chunk. The other rule, very short and cites nothing, is TERMINAL: it describes the opening words of every honest answer, so it cannot run per-chunk. That was the one real hole, and it closes exactly - the rule only fires below 40 characters, so nothing is streamed until the answer passes 40 characters, and an answer that never gets there is never streamed at all. The threshold is exported and shared, since two copies would drift and silently reopen the gap. Measured live: the first piece arrives at 41 characters and the remaining 110 stream individually. What remains weakened is nothing in the injection guard - only an out-of-range citation visible for a moment before the final event corrects it. generateAnswer and streamAnswer share one finaliser, because two copies would drift until one path gained a guard the other missed. Streams are deliberately NOT retried: retrying is safe only while nothing has been shown. Two details that would have been silent: stream_options.include_usage, without which the spend ledger recorded nothing for every streamed answer, and SSE frame reassembly, since a network chunk can split a frame mid-JSON. Verified on a real stream at 158 frames with zero unparseable.',
        status: 'done',
      },
      {
